...
from ft import CMD_HELP, StartTime
from ft.events import register
from ft.utils import edit_or_reply, humanbytes, man_cmd
...
async def _(ping):
    ...
    await xx.edit(
        ...
        f"❤️‍🔥 `𝚂𝚝𝚊𝚝𝚞𝚜` - `%sms`\n"
        f"❤️‍🔥 `𝚄𝚙𝚝𝚒𝚖𝚎` - `{uptime}⏱️` \n"
        f"**🤖 𝙼𝚢 𝙼𝚊𝚜𝚝𝚎𝚛 𝚒𝚜** - [{user.first_name}](tg://user?id={user.id})" % (duration)
    )
...
CMD_HELP.update(
    {
        ...
        \n  •  **Function : **Untuk menunjukkan ping ft.\
        \n\n  •  **Syntax :** `{cmd}pong`\
...
CMD_HELP.update(
    {
        ...
        \n  •  **Function : **Untuk Mengetes kecepatan server ft.\
    "
    }
)
