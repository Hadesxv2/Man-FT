...
from ft import BOT_VER, BOTLOG_CHATID
from ft import CMD_HANDLER as cmd
from ft import CMD_HELP, HEROKU_API_KEY, HEROKU_APP_NAME, branch
from ft.utils import edit_or_reply, man_cmd, time_formatter
...
async def shutdown_bot(event):
    ...
    if BOTLOG_CHATID:
        await event.client.send_message(
            BOTLOG_CHATID,
            "**#SHUTDOWN** \n"
            "**Man-FT** telah di matikan!\nJika ingin menghidupkan kembali silahkan buka heroku",
        )
    await edit_or_reply(event, "**Man-FT Berhasil di matikan!**")
...
async def restart_bot(event):
    await edit_or_reply(event, "**Man-FT Berhasil di Restart**")
    if BOTLOG_CHATID:
        await event.client.send_message(
            BOTLOG_CHATID, "#RESTART \n" "**Man-FT Berhasil Di Restart**"
        )
    args = [sys.executable, "-m", "FT"]
...
async def reedme(event):
    await edit_or_reply(
        ...
        "\n✣ [FT Repo](https://github.com/mrismanaziz/Man-FT/blob/Man-FT/README.md)"
        "\n✣ [Video Tutorial](https://youtu.be/tTDaPKsGC1I)"
        "\n✣ [List Variabel Heroku untuk Man-FT](https://telegra.ph/List-Variabel-Heroku-untuk-Man-FT-09-22)"
        "\n✣ [Setup Guide - Basic](https://mrismanaziz.medium.com/cara-memasang-FT-telegram-repo-man-FT-deploy-di-heroku-c56d1f8b5537)"
        "\n✣ [Setup Guide - LastFM Module](https://telegra.ph/How-to-set-up-LastFM-module-for-Paperplane-FT-11-02)",
    )
...
async def raw(event):
    ...
    with io.BytesIO(str.encode(the_real_message)) as out_file:
        ...
        await edit_or_reply(
            event, "`Check the FT log for the decoded message data !!`"
        )
...
CMD_HELP.update(
    {
        ...
        \n\n  •  **Syntax :** `{cmd}sleep`\
        \n  •  **Function : **Biarkan Man-FT tidur selama beberapa detik \
    "
    }
)
...
CMD_HELP.update(
    {
        "repo": f"**Plugin : **`Repository Man-FT`\
        \n\n  •  **Syntax :** `{cmd}repo`\
        \n  •  **Function : **Menampilan link Repository Man-FT\
        \n\n  •  **Syntax :** `{cmd}string`\
        \n  •  **Function : **Menampilan link String Man-FT\
    "
    }
)
...
CMD_HELP.update(
    {
        "readme": f"**Plugin : **`Panduan Menggunakan FT`\
        \n\n  •  **Syntax :** `{cmd}readme`\
        \n  •  **Function : **Menyediakan tautan untuk mengatur FT dan modulnya\
    "
    }
)
...
CMD_HELP.update(
    {
        "restart": f"**Plugin : **`Restart Man-FT`\
        \n\n  •  **Syntax :** `{cmd}restart`\
        \n  •  **Function : **Untuk Merestart FT.\
    "
    }
)
